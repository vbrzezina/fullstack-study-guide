...
A static Markdown study guide (~72,000 words) for senior full-stack engineering interview preparation. There is no code, no build system, and no tests — the entire repo is documentation.

## Structure

Content is split across 8 files due to size, plus a README with the full table of contents and the 6-week study plan (the study plan lives in the README, not in a numbered section). Sections are ordered for progressive learning: JS fundamentals first, then TypeScript, then async, then the full frontend block, then backend (including messaging, DSA, and real-time), then DevOps & CI/CD, then observability & security, then cloud & data engineering, then ways of working (Git, engineering excellence, agile, docs), then solution architecture.
